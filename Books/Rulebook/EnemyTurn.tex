...
\begin{tcolorbox}[colframe=blue, colback=white]
	Reminder - Synchronising the transition is important since you may still receive help from an ally after you played all of your cards.
\end{tcolorbox}
...
\begin{figure}[ht]
	\begin{minipage}[t][5cm][t]{0.5\textwidth}
		...
		\begin{enumerate}
			...
			\item \textbf{Passives} - Enemies can have passive effects too \textit{(See "Enemy Keywords", on handout)}.
			...
			\item \textbf{Actions} - enemies act according to the element that currently points at you on the wheel\\
		\end{enumerate}
...
To perform the enemy turn, look at the \textbf{element} on the wheel which currently points at you  and have the enemy perform the corresponding action.
