...
\begin{enumerate}	
	...
	\item \textbf{Ancestry Tableau} - The tableau contains an overview for the combat turn, as well as helping you track your decisions as you level up and your health pool.
	\item \textbf{Enemies} - Enemies are placed next to the wheel, with the configuration described in the storybook.
	...
	\item \textbf{Storybook} - This book contains the story and encounter setup.
	\item \textbf{Event Deck} - Between encounters, events will be drawn from this deck to mix up the way the following encounters unfold.
	\item \textbf{Markers} - Cubes for tracking the enemy life, and markers to track boons \& banes.
	\item \textbf{Enemy Die} - Determines the enemy actions.
\end{enumerate}
\end{minipage}	
...
\begin{tcolorbox}[colframe=blue, colback=white]
	Reminder -	If you are overwhelmed by the choices in the next section, check the beginner recommendations for Ancestries \& Classes .  \textit{(See "Ancestry \& Class Overview", Page\pageref{Section_AncestryClassOverview})}.
\end{tcolorbox}
...
After you beat an encounter, you draw an event, then level up \textit{(See "Events and Leveling", Page \pageref{Section_EventsAndLeveling})}.
